  ...
  return (
    <div ref={boxRef} className="relative">
      <input
        ...
        className="w-44 lg:w-60 bg-white/15 text-white placeholder-white/60 rounded-full
                   px-4 py-1.5 text-sm font-body outline-none border border-white/20
                   focus:bg-white focus:text-gray-900 focus:placeholder-gray-400 focus:border-gold transition-colors"
      />

      {open && query && (
        <div className="absolute right-0 mt-1 w-72 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden z-50 text-left">
          {!hasResults && (
          ...
          {parishes.length > 0 && (
            <div className="py-1">
              <p className="px-4 pt-1 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Parishes</p>
              {parishes.map(p => (
          ...
          {churches.length > 0 && (
            <div className="py-1 border-t border-gray-100">
              <p className="px-4 pt-1 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-400">Churches</p>
              {churches.map(c => (
                <a key={c.id} href={to(`/church/${c.id}`)} onClick={close}
                   className="block px-4 py-2 hover:bg-ivory text-sm font-body">
                  <span className="font-semibold text-gray-900">{c.name}</span>
                  <span className="text-xs text-gray-500 ml-1">— {c.town ? `${c.town}, ` : ''}{c.parish}</span>
                </a>
              ))}
            </div>
          )}
